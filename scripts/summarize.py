...
1. 总计 8-12 条，按类别分组：
   ...
   ## 📰 行业综合
2. 每条格式固定如下（示例）：
- **山东淄博电梯安装坍塌事故调查报告公布 1死2伤**：事故造成1人死亡、2人受伤，官方调查报告已正式对外公布。
  - 重要性：★★★★☆ / 5
  - AI摘要：当前新闻的摘要，不超过300字
...
6. 标题格式统一为 **加粗标题**：简述，不要在标题前添加分类标签如 [事故通报]、[监管提示] 等"""
...
7. 标题格式统一为 **加粗标题**：简述，不要在标题前添加分类标签如 [事故通报]、[监管提示] 等
"""
...
3. 字段标签翻译为日语（重要性 → 重要度，AI摘要 → AI要約，来源 → 出典 等）
